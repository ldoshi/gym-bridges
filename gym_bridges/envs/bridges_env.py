  ...
  def reset(self, state=None, gap_count=None):
    if state is not None:
      ...
      for x in range(state_width + 1):
        if x < state_width and state[state_base_height-1][x] == StateType.GROUND:
          width += 1
        else:
          # End of block. Compute height and save block.
          height = 0
          while state[state_base_height - height - 1][x-1] == StateType.GROUND:
            height += 1
      ...
      for initial_block in self._initial_blocks:
        for env_index in range(initial_block.index, initial_block.index + initial_block.width):
          for height_adjustment in range(1, initial_block.height + 1):
            self.state[self.shape[0]-height_adjustment][env_index] = StateType.GROUND

    base_height = self.shape[0]
    self._starting_block_surface = [(base_height - self._initial_blocks[0].height, self._initial_blocks[0].index + x) for x in range(self._initial_blocks[0].width)]
    self._ending_block_surface = set([(base_height - self._initial_blocks[-1].height, self._initial_blocks[-1].index + x) for x in range(self._initial_blocks[-1].width)])

    self._central_block_surfaces = []
    for i in range(1, len(self._initial_blocks) - 1):
      self._central_block_surfaces.append([(base_height - self._initial_blocks[i].height, self._initial_blocks[i].index + x) for x in range(self._initial_blocks[i].width)])

    return self.state.copy()
      
  def render(self, mode='human'):
    print((("%s"*self.shape[1]+"\n")*self.shape[0]) % tuple(["@@" if x == StateType.GROUND else "[]" if x == StateType.BRICK else "  " for x in self.state.flatten()]))
