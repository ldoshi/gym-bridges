...
def _check_block(state, index, width_check_value):
    """Verifies that the block start at index is grounded in the environment and maintains a level height until the next gap. Returns the index following this block on success. Gaps are checked as blocks of height 0."""
...
class TestBridgesEnv(unittest.TestCase):
    ...
    def test_force_standard_config(self, width):
        """The standard configuration only has a value of 1 at the bottom left and right locations of the env."""
    ...
    def test_gap_count(self, width):
        """Verifies the requested number of gaps exist. This test also verifies that the env is well-formed in terms of having initial blocks of ground (1 values) properly grounded to the bottom of the env and level between gaps."""        
    ...
    def test_max_gap_count(self, width):
        ...
        for gap_count in range(1, ((width + 1) // 2)):
            ...
            for _ in range(100):
                self.env.reset()
                counts[int(np.sum([self.env.state[height][i] if self.env.state[height][i] - self.env.state[height][i+1] else 0 for i in range(self.env.state.shape[1] -1)]))] += 1

            self.assertEqual(len(counts), gap_count)

    def test_is_bridge_complete_simple(self):
        """Verifies bridge completion detection in a very simple set-up."""
        self.env.setup(width=4)
